...
def get_facial_color(image):
    image = imutils.resize(image, width=250)
    skin = extractSkin(image)
    dominantColors = extractDominantColor(skin, hasThresholding=True)
    dominantColors = dominantColors[0].get('color')
    final_colors = tuple(dominantColors)
    actual_name, closest_name = get_colour_name(final_colors)
    color = colors.get(closest_name)
    return color
